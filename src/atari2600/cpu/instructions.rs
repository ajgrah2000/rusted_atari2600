...
const ADDR_ACCUMULATOR: addressing::AddressingAccumulator =
    addressing::AddressingAccumulator::new();
...
const ADDR_ABY_PAGE_DELAY: addressing::AddressingAbyPageDelay =
    addressing::AddressingAbyPageDelay::new();
const ADDR_ABX_PAGE_DELAY: addressing::AddressingAbxPageDelay =
    addressing::AddressingAbxPageDelay::new();
...
impl Instruction {
    pub fn execute(
        ...
        let op_instruction = |op: u8| match op {
                n if get_c(n) == 0x1 => {
        ...
        match op_code {
            0xEA => {
                instruction_set::single_byte_instruction(
                    clock,
                    pc_state,
                    memory,
                    READ_REG_A,
            ...
            0x00 => {
                instruction_set::break_instruction(clock, pc_state, memory);
            }
            ...
            n if n & 0x3 == 0x1 => {
                match op_code {
                    // EOR
                    0x41 => {
                        instruction_set::read_write_instruction(
                            clock,
                            pc_state,
                            memory,
                            &ADDR_IZX,
                    ...
                    0x9D => {
                        instruction_set::read_write_instruction_additional_delay(
                            clock,
                            pc_state,
                            memory,
                            &ADDR_ABX,
                    ...
                    _ => {
                        panic!("Unmatched (0xXXXXXX01) opcode 0x{:x}", op_code);
                    }
                }
            }
            ...
            // JSR
            0x20 => {
                instruction_set::jump_sub_routine_instruction(clock, pc_state, memory);
            }

            // BPL case 0x10: if (self.pc_state.P.status.N == 0)
            0x10 => {
                instruction_set::branch_instruction(clock, pc_state, memory, 0x80, 0x00);
            }
            // BMI case 0x30: if (self.pc_state.P.status.N == 1)
            0x30 => {
                instruction_set::branch_instruction(clock, pc_state, memory, 0x80, 0x80);
            }
            // BVC case 0x50: if (self.pc_state.P.status.V == 0)
            0x50 => {
                instruction_set::branch_instruction(clock, pc_state, memory, 0x40, 0x00);
            }
            // BVS case 0x70: if (self.pc_state.P.status.V == 1)
            0x70 => {
                instruction_set::branch_instruction(clock, pc_state, memory, 0x40, 0x40);
            }
            // BCC case 0x90: if (self.pc_state.P.status.C == 0)
            0x90 => {
                instruction_set::branch_instruction(clock, pc_state, memory, 0x01, 0x00);
            }
            // BCS case 0xB0: if (self.pc_state.P.status.C == 1)
            0xB0 => {
                instruction_set::branch_instruction(clock, pc_state, memory, 0x01, 0x01);
            }
            // BNE case 0xD0: self.clocks += 2*CYCLES_TO_CLOCK if (self.pc_state.P.status.Z == 0)
            0xD0 => {
                instruction_set::branch_instruction(clock, pc_state, memory, 0x02, 0x00);
            }
            // BEO case 0xF0: if (self.pc_state.P.status.Z == 1)
            0xF0 => {
                instruction_set::branch_instruction(clock, pc_state, memory, 0x02, 0x02);
            }

            0x40 => {
                instruction_set::return_from_interrupt(clock, pc_state, memory);
            }
            // RTS
            0x60 => {
                instruction_set::return_from_sub_routine_instruction(clock, pc_state, memory);
            }

            // JMP, absolute (effectively immediate)
            0x4C => {
                instruction_set::jump_instruction(clock, pc_state, memory, &ADDR_ABS);
            }
            // JMP, absolute (effectively absolute)
            0x6C => {
                instruction_set::jump_instruction(clock, pc_state, memory, &ADDR_INDIRECT);
            }

            // PHP
            0x08 => {
                instruction_set::php_instruction(clock, pc_state, memory);
            }

            // PLP
            0x28 => {
                instruction_set::plp_instruction(clock, pc_state, memory);
            }

            // PHA
            0x48 => {
                instruction_set::pha_instruction(clock, pc_state, memory);
            }

            // PLA
            0x68 => {
                instruction_set::pla_instruction(clock, pc_state, memory);
            }
