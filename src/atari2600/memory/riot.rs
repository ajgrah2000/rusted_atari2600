...
impl Riot {
    ...
    pub fn read(&mut self, clock: &clocks::Clock, address: u16) -> u8 {
        ...
        } else if test == Riot::RIOT_TIM1T
            || test == Riot::RIOT_TIM8T
            || test == Riot::RIOT_TIM64T
            || test == Riot::RIOT_T1024T
            || test == Riot::TIMERADDR
        {
            if self.expiration_time >= future_clock {
                // If expiration hasn't occured, return the time remaining.
                value = ((self.expiration_time - future_clock)
                    / (self.interval as clocks::ClockType * Riot::CYCLES_TO_CLOCK))
                    as u8;
            } else {
                // Calculate ticks past zero, may not be quite right
                // The interval was passed, value counts down from 255.
                value = (0x100 as i16).wrapping_sub(
                    ((future_clock - self.expiration_time) / Riot::CYCLES_TO_CLOCK) as i16,
                ) as u8;
            }
    ...
    pub fn write(&mut self, clock: &clocks::Clock, address: u16, data: u8) {
        if 0 == (address & Riot::NOT_RAMSELECT) {
            ...
            self.expiration_time = clock.ticks
                + Riot::CYCLES_TO_CLOCK
                    * data as clocks::ClockType
                    * self.interval as clocks::ClockType;
        }
    }
}
