...
pub fn single_byte_instruction<
    R,
    W,
    I: Fn(&mut clocks::Clock, &mut pc_state::PcState, &mut memory::Memory, u8) -> u8,
>(
    clock: &mut clocks::Clock,
    pc_state: &mut pc_state::PcState,
    memory: &mut memory::Memory,
...
{
    read_write_instruction_additional_delay(
        clock,
        pc_state,
        memory,
        address,
...
pub fn read_write_instruction_additional_delay<
    A,
    R,
    W,
    I: Fn(&mut clocks::Clock, &mut pc_state::PcState, &mut memory::Memory, u8) -> u8,
...
pub fn jump_sub_routine_instruction(
    clock: &mut clocks::Clock,
    pc_state: &mut pc_state::PcState,
    memory: &mut memory::Memory,
) {
...
pub fn return_from_sub_routine_instruction(
    clock: &mut clocks::Clock,
    pc_state: &mut pc_state::PcState,
    memory: &mut memory::Memory,
) {
...
pub fn branch_instruction(
    ...
    if (pc_state.get_p() & condition_mask) == condition {
        ...
        branch_page_delay(
            ...
            pc_state.get_pc().wrapping_add(2),
        );
...
pub fn adc(
    ...
    let result = add_carry(
        pc_state,
        pc_state.get_a(),
        data,
        pc_state.get_flag_c() as u8,
    );
...
pub fn sub_carry(pc_state: &mut pc_state::PcState, a: u8, b: u8, c: u8) -> u8 {
    ...
    if false == pc_state.get_flag_d() {
        ...
        let r = (((a >> 4) & 0xF) * 10 + ((a & 0xF) % 10)) as i16
            - (((b >> 4) & 0xF) * 10 + ((b & 0xF) % 10)) as i16
            - c as i16;
