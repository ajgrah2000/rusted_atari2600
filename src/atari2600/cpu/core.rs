...
impl Core {
    ...
    pub fn reset(&mut self) {
        ...
        self.pc_state
            .set_pc(self.memory.read16(&self.clock, Core::PROGRAM_ENTRY_ADDR));
    }
    ...
    pub fn step(&mut self, debug: bool, realtime: bool) {
        if realtime {
            ...
            if 1000 * self.clock.ticks as u64 / Constants::CLOCK_HZ as u64 > in_ms as u64 {
                let required_sleep =
                    (1000 * self.clock.ticks as u64 / Constants::CLOCK_HZ as u64) - in_ms;
        ...
        if debug {
            print!(
                ...
                (self
                    .clock
                    .ticks
                    .wrapping_sub(self.memory.stella.debug_clock()))
                    / pc_state::PcState::CYCLES_TO_CLOCK as u64,
                op_code,
                self.pc_state.get_pc(),
                self.memory
                    .read(&self.clock, self.pc_state.get_pc().wrapping_add(1))
            );
