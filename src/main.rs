...
struct RustAtari2600Args {
    ...
    #[argh(
        option,
        short = 'c',
        default = "atari2600::memory::cartridge::CartridgeType::Default",
        from_str_fn(parse_cartridge)
    )]
    cartridge_type: atari2600::memory::cartridge::CartridgeType,
}

fn cartridge_type_help_fn() -> String {
    atari2600::memory::cartridge::CartridgeType::iter()
        .into_iter()
        .fold("cartridge type: ".to_owned(), |all, value| {
            format!("{} {:?}", all, value)
        })
}
...
fn main() {
    ...
    let mut atari_machine = atari2600::atari2600::Atari2600::new(
        args.debug,
        !args.no_delay,
